...
Web 端运行时从 **`VITE_KANSHAN_MODEL_WEB_URL`** 拉取压缩版 GLB，不会打进 Vite 产物；请勿在仓库内公开具体 CDN，值只在部署环境与本地 `.env.local`（已 gitignore）中配置。
...
将生成的 `*-web.glb` 上传到你控制的存储后，再把该 HTTPS 地址写到 `VITE_KANSHAN_MODEL_WEB_URL`。
