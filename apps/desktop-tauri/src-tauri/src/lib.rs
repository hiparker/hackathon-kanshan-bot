...
fn is_allowed_external_url(url: &str) -> bool {
    url == "https://zhida.ai/"
        ...
        || is_https_public_page_url(url)
}

/// 「查看原文」等外链：允许无用户信息的公网 https（由用户点击触发，经系统浏览器打开）。
fn is_https_public_page_url(url: &str) -> bool {
    const PREFIX: &str = "https://";
    if !url.starts_with(PREFIX) || url.len() > 4096 {
        return false;
    }
    ...
    !(host_lc == "localhost" || host_lc.starts_with("127."))
}
...
/// 多屏混合 DPI 下，全局光标与 inner 坐标的对应关系在部分系统上仍会误判，导致整窗长期 ignore 鼠标。
/// 多块屏时关闭 OS 穿透（透明区不再点到桌面下层）；单块屏仍保留穿透。
/// 强制关闭：启动前设置环境变量 `KANSHAN_DISABLE_CURSOR_PASSTHROUGH=1`。
fn spawn_cursor_passthrough_loop(window: WebviewWindow) {
    tauri::async_runtime::spawn(async move {
        let mut last_ignore: Option<bool> = None;
        // 连续读坐标失败时强制关闭穿透，避免 macOS 在 ignoreCursorEvents 下偶发读不到鼠标而卡死。
        let mut coord_error_streak: u32 = 0;
        let mut passthrough_poll_tick: u32 = 0;
        let mut use_os_cursor_passthrough: bool = true;
        loop {
            ...
            if passthrough_poll_tick % 64 == 1 {
                ...
                let multi_display = window
                    ...
                    .unwrap_or(false);
                use_os_cursor_passthrough = !user_off && !multi_display;
            }

            let scale = window.scale_factor().unwrap_or(1.0);
            // 与前端 getBoundingClientRect * scale 一致：相对客户区物理像素；outer_* 与内层坐标易错位（多屏/无边框）。
            let coord_slack = f64::max(14.0, scale * 10.0);

            let cursor = match window.cursor_position() {
                Ok(pos) => pos,
                Err(_) => {
                    coord_error_streak = coord_error_streak.saturating_add(1);
                    last_ignore = None;
                    if coord_error_streak >= 2 {
                        let _ = window.set_ignore_cursor_events(false);
                        last_ignore = Some(false);
                        coord_error_streak = 0;
                    }
                    continue;
                }
            };

            let (win_pos, win_size) = match (window.inner_position(), window.inner_size()) {
                (Ok(p), Ok(s)) => (p, s),
                (Err(_), _) | (_, Err(_)) => match (window.outer_position(), window.outer_size()) {
                    (Ok(p), Ok(s)) => (p, s),
                    _ => {
                        coord_error_streak = coord_error_streak.saturating_add(1);
                        last_ignore = None;
            ...
            let in_window = local_x >= -coord_slack
                && local_x <= width + coord_slack
                && local_y >= -coord_slack
                && local_y <= height + coord_slack;
            ...
            let in_stage = local_x >= stage_bounds.x - coord_slack
                && local_x <= stage_bounds.x + stage_bounds.width + coord_slack
                && local_y >= stage_bounds.y - coord_slack
                && local_y <= stage_bounds.y + stage_bounds.height + coord_slack;
            let in_dom_region = interactive_regions.iter().any(|region| {
                local_x >= region.x - coord_slack
                    && local_x <= region.x + region.width + coord_slack
                    && local_y >= region.y - coord_slack
                    && local_y <= region.y + region.height + coord_slack
            });

            let interactive = passthrough_suppressed || (in_window && (in_stage || in_dom_region));
            let should_ignore = use_os_cursor_passthrough && !interactive;
